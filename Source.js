...
const pigMe = str => str.split(/([^a-zA-Z'])/).map(c =>/[a-zA-Z]+/.test(c) ? c.slice(1)+c[0] +'ay': c).join('')
const unPigMe = str =>  str.split(/([^a-zA-Z'])/).map(c =>/[a-zA-Z]+/.test(c) ? c.charAt(c.length-3) + c.slice(0,-3): c).join('')
